-- Fix email harvesting vulnerability by removing organization member access to profiles
-- and creating a secure function for non-sensitive organization member data
...
-- Drop the problematic policy that allowed organization member access to emails
DROP POLICY IF EXISTS "Secure function access only" ON public.profiles;

-- Create a more secure policy that completely removes organization member access
CREATE POLICY "Secure access for profiles"
ON public.profiles
FOR SELECT
USING (
