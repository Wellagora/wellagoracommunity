...
import { Store, BookOpen, Bell, User } from "lucide-react";
...
const MobileBottomNav = () => {
  ...
  const hiddenPaths = ["/auth", "/admin-panel", "/szakertoi-studio", "/tamogatoi-kozpont"];
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className="flex flex-col items-center justify-center flex-1 h-full min-w-[64px] min-h-[44px] relative group"
            >
              <motion.div
